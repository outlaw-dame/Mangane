...
const expectedUnknowns = [
  ...
  'Production edge rewrite precedence for the scoped share-target path still requires deployment verification.',
];
...
const expectedRegistration = [
  ...
  'navigator.serviceWorker.register(`${scope}share_target.js`,{scope,});',
];
validateExactList(registration.requiredExecutableBindings, expectedRegistration, 'development registration binding');
const registrationSource = compactExecutable(readInsideRoot(registration.path, 'registration'));
for (const binding of expectedRegistration) {
  const normalizedBinding = compactExecutable(binding);
  if (!registrationSource.includes(normalizedBinding)) fail(`${registration.path} no longer registers the expected scoped share-target worker: ${binding}`);
}

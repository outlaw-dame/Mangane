...
- These observations do not prove that the repository has no workflows; alternate filenames, external CI, disabled workflows, branch-only workflows, and unenumerated configuration remain possible.
...
- baseline command outcomes on the current repository state;
...
Every discovered workflow and job must be entered into this matrix. Missing coverage must remain explicit rather than being inferred from package scripts.
...
This baseline is incomplete until every workflow, trigger, job, runtime, permission, secret, cache, artifact, required check, and baseline outcome is directly enumerated. Package scripts alone are not evidence that CI runs or enforces them.
